...
define('CONTENTIFY_AI_NONCE_NAME', 'cai_nonce');
define('CONTENTIFY_AI_NONCE_BN', basename(__FILE__));
define('CONTENTIFY_AI_API_BASE_URL', 'https://api.contentify.app/' );
define('CONTENTIFY_AI_VER', '1.4.6');
	...
	class Contentify_AI_Class
	{
		...
		public function create_api_key_authentication()
		{
			...
			register_rest_route(
				...
				array(
					'methods' => 'POST',
					'callback' => array($this, 'create_page_with_api_key'),
					'permission_callback' => '__return_true',
				)
			);
		}
...
// Callback function to fetch all the blogs
		public function get_all_blogs($data)
		{
			...
			if ($query->have_posts()) {
				while ($query->have_posts()) {
					...
					$thumbnail_url = $thumbnail_data ? $thumbnail_data[0] : false;
					...
					$blog_data = array(
						...
						'thumbnail_url' => $thumbnail_url, // Add the thumbnail URL
					);
		...
		public function create_page_with_api_key($data) {
			$author = $data->get_param('author');
			$api_key = $data->get_param('api_key');
			$title = $data->get_param('title');
			$status = $data->get_param('status');
			...
			$status = strtolower($status);
			if (!isset(get_page_statuses()[$status]))
				$status = 'publish';
		...
		public function register_contentify_settings()
		{
			...
			add_settings_field(
				'contentify_api_link',
				null,
				array($this, 'contentify_api_link_callback'),
				'contentify_settings',
				'contentify_settings_section'
			);
		}
		...
		public function contentify_api_link_callback()
		{
			echo sprintf( __( '%1$sOptimize Unoptimized Pages%2$s', 'contentify-ai'), '<a class="button" href="' . esc_attr( add_query_arg( 'skip', '1', menu_page_url( 'contentify-ai-ob', false ) ) ) . '">', '</a>' );
		}
